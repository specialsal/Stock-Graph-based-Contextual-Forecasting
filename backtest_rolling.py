...
def main():
    ...
    with h5py.File(daily_h5, "r") as h5:
        ...
        for i in range(train_weeks,
            ...
            for ep in range(CFG.epochs_warm):
                ...
                for fd, ind, ctx, y in pbar:
                    ...
                    with torch.no_grad():
                        ...
                        # NaN 防御
                        icp = 0.0 if math.isnan(icp) else icp
                        icr = 0.0 if (icr is None or not math.isfinite(icr)) else icr
                        icp_sum += icp * bs
                        icr_sum += icr * bs
                ...
                def eval_split_ram(mem_items):
                    ...
                    for it in mem_items:
                        ...
                        ric  = 0.0 if (ric is None or not math.isfinite(ric)) else float(ric)
                        ...
                        cc = 0.0 if math.isnan(cc) else cc
                ...
                if len(per_date_val) == 0:
                    ...
                    # NaN 兜底
                    avg_icp = float(np.nan_to_num(avg_icp, nan=0.0))
                    avg_icr = float(np.nan_to_num(avg_icr, nan=0.0))
            ...
            alpha_sel = getattr(CFG, "score_alpha", 0.5)
            recentN = getattr(CFG, "recent_topN", 5)
            df = df_all.copy()

            # 规范 pred_date & 数值列
            df["pred_date"] = pd.to_datetime(df["pred_date"], errors="coerce")
            for col in ["test_avg_rankic", "test_std_rankic"]:
                df[col] = pd.to_numeric(df[col], errors="coerce")

            # 计算 score_test
            df["score_test"] = df["test_avg_rankic"] - alpha_sel * df["test_std_rankic"]

            # 仅在 score_test 有限值上进行选择
            mask_valid = np.isfinite(df["score_test"].to_numpy(dtype="float64"))
            df_valid = df.loc[mask_valid].copy()

            best_overall = None
            if len(df_valid) > 0:
                idx_o = df_valid["score_test"].idxmax()
                best_overall = df_valid.loc[idx_o].to_dict()
                if isinstance(best_overall.get("best_path", None), str) and Path(best_overall["best_path"]).exists():
                    dst = CFG.model_dir / "best_overall.pth"
                    ensure_parent_dir(dst)
                    shutil.copy2(best_overall["best_path"], dst)
                else:
                    print("[选择] 全局最优 best_path 不存在，跳过复制。")
            else:
                print("[选择] 警告：score_test 全为 NaN/非有限值，跳过全局最优复制。")

            best_recent = None
            if recentN > 0 and len(df_valid) > 0:
                df_recent = df_valid.sort_values("pred_date").tail(recentN)
                if len(df_recent) > 0:
                    idx_r = df_recent["score_test"].idxmax()
                    best_recent = df_recent.loc[idx_r].to_dict()
                    if isinstance(best_recent.get("best_path", None), str) and Path(best_recent["best_path"]).exists():
                        dst = CFG.model_dir / f"best_recent_{recentN}.pth"
                        ensure_parent_dir(dst)
                        shutil.copy2(best_recent["best_path"], dst)
                    else:
                        print("[选择] 最近最优 best_path 不存在，跳过复制。")
                else:
                    print("[选择] 提示：recentN 切片为空，跳过最近最优复制。")
            else:
                if recentN <= 0:
                    print("[选择] recent_topN<=0，跳过最近最优复制。")
                elif len(df_valid) == 0:
                    print("[选择] 无有效记录（score_test 非有限），跳过最近最优复制。")
