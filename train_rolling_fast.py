...
# coding: utf-8
"""
滚动训练主循环（日频 + 行业图/关闭）- 窗口级一次性常驻内存版
- 每个窗口开始时，将 Train/Val 全部 group 一次性读入内存（常驻至窗口结束），epoch 内零 IO
- 仅窗口切换时才从 HDF5 读取一次，充分利用 64GB 内存
- 验证保持按日期聚合指标
- HDF5 打开时启用较大的 raw data chunk cache
- 修复无 CUDA 时 .pin_memory() 报错：仅在 has_cuda=True 时使用
"""
import os, platform, torch, pandas as pd, numpy as np, h5py, shutil, math, random
...
def load_window_to_ram(
    ...
    """
    将一个窗口的所有 group 一次性加载到内存，返回列表 items 和统计信息。
    items[i] = dict(date, X, ind, ctx, y)，按 group_keys 的相对顺序保存。
    """
    ...
    print(f"[预载入-常驻] 已加载 {len(items)} 个 group 到内存，共 {total_bytes/1e9:.2f} GB（跳过空组 {skipped}）")
    return items

def iterate_ram_minibatches(mem_items: List[Dict[str, Any]], batch_size: int, shuffle: bool = True):
    """
    仅使用常驻内存的数据进行训练迭代：打乱 group 顺序，每个 group 内再打乱/切 batch。
    """
    if not mem_items:
        return
    order = list(range(len(mem_items)))
    if shuffle:
...
# --------------------------- 训练（完全在常驻内存上） --------------------------- #
def train_one_epoch_from_ram(model, mem_items, opt, scaler_grad, amp_dtype, print_interval=10):
    model.train()
    ...
    pbar = tqdm(iterate_ram_minibatches(mem_items, CFG.batch_size, shuffle=True),
                total=None, leave=False, desc="train-fast[RAM]")
...
# --------------------------- 验证（使用常驻内存，逐组聚合） --------------------------- #
@torch.no_grad()
def evaluate_from_ram(model, mem_items_val):
    model.eval()
    per_date = []
    # mem_items_val 按组存储，逐组评估便于统计日期级指标
    for it in mem_items_val:
        X = torch.from_numpy(it["X"]).to(CFG.device, non_blocking=HAS_CUDA)
        ind = torch.from_numpy(it["ind"]).to(CFG.device, non_blocking=HAS_CUDA)
        ctx = torch.from_numpy(it["ctx"]).to(CFG.device, non_blocking=HAS_CUDA)
        y  = torch.from_numpy(it["y"]).to(CFG.device, non_blocking=HAS_CUDA)

        preds = []
        bs = CFG.batch_size
        for st in range(0, X.shape[0], bs):
            preds.append(model(X[st:st+bs], ind[st:st+bs], ctx[st:st+bs]).detach().float().cpu())
...
def main():
    ...
    # 4. 维度（使用大缓存打开 HDF5 仅用于“首次窗口载入”）
    with h5py.File(daily_h5, "r",
                   rdcc_nbytes=int(getattr(CFG, "h5_rdcc_nbytes", 512*1024*1024)),
                   rdcc_nslots=int(getattr(CFG, "h5_rdcc_nslots", 1_000_003)),
                   rdcc_w0=float(getattr(CFG, "h5_rdcc_w0", 0.75))) as h5d:
    ...
    with h5py.File(daily_h5, "r",
                   rdcc_nbytes=int(getattr(CFG, "h5_rdcc_nbytes", 512*1024*1024)),
                   rdcc_nslots=int(getattr(CFG, "h5_rdcc_nslots", 1_000_003)),
                   rdcc_w0=float(getattr(CFG, "h5_rdcc_w0", 0.75))) as h5:
        for i in range(CFG.train_years * 52,
            ...
            # HDF5 中 group 的 key 假设为 date_0 ... 连续
            ...
            # 仅在窗口开始时：一次性加载 Train/Val 到内存（常驻到窗口结束）
            mem_items_train = load_window_to_ram(h5, train_gk, label_df, ctx_df, scaler_d, ind_map, pad_ind_id, filter_fn=filter_fn)
            mem_items_val   = load_window_to_ram(h5, val_gk,   label_df, ctx_df, scaler_d, ind_map, pad_ind_id, filter_fn=filter_fn)

            # 训练 & 评估（epoch 内完全复用内存数据）
            best_metric = -1e9
            best_epoch  = -1
            best_path   = CFG.model_dir / f"model_best_{pred_date.strftime('%Y%m%d')}.pth"

            for ep in range(CFG.epochs_warm):
                trm = train_one_epoch_from_ram(
                    model, mem_items_train, opt, scaler_grad, _get_amp_dtype(),
                    print_interval=getattr(CFG, "print_step_interval", 10)
                )
                ...
                valm = evaluate_from_ram(model, mem_items_val)
            ...
            final_val = evaluate_from_ram(model, mem_items_val)
